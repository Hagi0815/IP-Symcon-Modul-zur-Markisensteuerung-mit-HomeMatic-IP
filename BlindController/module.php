...
class Rolladensteuerung extends IPSModuleStrict
{
    ...
    private function applyContactLogic(array $positionsAct, array $positionsNew, int $deactivationTimeAuto, bool $bNoMove, string $Hinweis, int $priorityMode = 1): array
    {
        ...
        $openBlindResult            = $this->getPositionsOfOpenBlindContact();
        $positionsContactOpenBlind  = $openBlindResult !== null ? $openBlindResult['positions'] : null;
    ...
    /**
     * Wertet den Zustand aller Kontakte aus und gibt die resultierenden Blindpositionen zurueck.
     *
    ...
    private function getBlindPositions(array $contacts): ?array
    {
        ...
        foreach ($contacts as $propName => $contact) {
            // Aktiven Zustand ermitteln (0 = zu, 1 = gekippt, 2 = geoeffnet)
            $state = $this->getContactState($propName, $contact);

            if ($state === 0) {
                continue; // Kontakt geschlossen, ignorieren
            }

            $contactOpen = true;

            // Positionen fuer diesen Kontakt waehlen
            if ($state === 1) {
    ...
    /**
     * Ermittelt den Zustand eines Kontakts als Integer:
     *   0 = geschlossen
     *   1 = gekippt  (nur bei Integer-Modus)
     *   2 = geoeffnet
     *
     * @param string $propName  Property-Name des Kontakts (fuer isContactOpen-Fallback)
     * @param array  $contact   Kontakt-Konfigurationsdaten aus getDefinedContacts()
     * @return int
     */
    private function getContactState(string $propName, array $contact): int
    {
        if ($contact['useInteger']) {
            // Integer-Modus: Rohwert auslesen und mit Schwellwerten vergleichen
            $rawValue  = (int)GetValue($contact['id']);
            $tiltValue = $contact['tiltValue'];
            $openValue = $contact['openValue'];

            $this->Logger_Dbg(
                __FUNCTION__,
                sprintf(
                    '%s (#%s): Integer-Modus, Wert=%d, Kipp-Schwelle=%d, Oeffnen-Schwelle=%d',
                    $propName, $contact['id'], $rawValue, $tiltValue, $openValue
                )
            );
            ...
            if ($rawValue >= $tiltValue) {
                return 1; // gekippt
            }
            return 0; // geschlossen
        }

        // Boolean-/Profil-Modus: bestehende Logik verwenden
        return $this->isContactOpen($propName) ? 2 : 0;
    }

    /**
     * Gibt die Blindpositionen und den Zustandshinweis fuer aktive Oeffnen-Kontakte zurueck.
     * Rueckgabe: ['positions' => [...], 'hint' => 'Kontakt gekippt'|'Kontakt geoeffnet'] oder null.
     */
    ...
    /**
     * Wie getBlindPositions(), gibt aber zusaetzlich einen Zustandshinweis zurueck.
     */
    private function getBlindPositionsWithHint(array $contacts): ?array
    {
        $contactOpen    = null;
        $blindPositions = null;
        $maxState       = 0; // 0=zu, 1=gekippt, 2=geoeffnet
